...
import com.techjumper.polyhomeb.adapter.recycler_ViewHolder.databean.MessageOrderBean;
...
public class MessageOrdersFragmentModel extends BaseModel<MessageOrdersFragmentPresenter> {
    ...
    public void updateData(MessageEntity entity) {
        ...
        for (int i = 0; i < messages.size(); i++) {
            ...
            //第三个item,内容
            MessageOrderData messageOrderData = new MessageOrderData();
            messageOrderData.setContent(content);
            messageOrderData.setTypes(messagesBean.getTypes());
            messageOrderData.setRightText(rightText);
            messageOrderData.setTime(time);
            messageOrderData.setHas_read(messagesBean.getHas_read());
            messageOrderData.setTitle(title);
            messageOrderData.setId(id);
            messageOrderData.setObj_id(obj_id);
            MessageOrderBean messageOrderBean = new MessageOrderBean(messageOrderData);
            mDataList.add(messageOrderBean);
