...
public class ComplainDetailActivityPresenter extends AppBaseActivityPresenter<ComplainDetailActivity> {
    ...
    public List<DisplayBean> getReplyDatas(List<ComplainDetailEntity.DataBean.RepliesBean> replies) {
    ...
    private void getComplainDetailData() {
        ...
        addSubscription(
                mSubs2 = mModel.getComplainDetail()
                        .subscribe(new Observer<ComplainDetailEntity>() {
                            @Override
                            public void onCompleted() {
                                getView().dismissLoading();
                            }
                            ...
                            @Override
                            public void onNext(ComplainDetailEntity complainDetailEntity) {
                                if (!processNetworkResult(complainDetailEntity)) return;
                                getView().onComplainDataReceive(complainDetailEntity);
                                getView().dismissLoading();
                            }
                        }));
    }
        ...
        addSubscription(
                mSubs3 = mModel.complainDetailReply(content)
                        .subscribe(new Observer<TrueEntity>() {
                            ...
                            public void onError(Throwable e) {
                                getView().showError(e);
                                addMessageToRvList(content, true);
                            }
                            ...
                            public void onNext(TrueEntity trueEntity) {
                                ...
                                if (Constant.TRUE_ENTITY_RESULT.equals(trueEntity.getData().getResult())) {
                                    ToastUtils.show(getView().getString(R.string.send_success));
                                    addMessageToRvList(content, false);
                                } else {
                                    addMessageToRvList(content, true);
                                }
    ...
    private void addMessageToRvList(String content, boolean isField) {
        //添加消息到mModel中
        ...
        propertyRepairDetailProprietorContentData.setFailed(isField);
    ...
    public List<DisplayBean> alreadyChoosedPic(ComplainDetailEntity complainDetailEntity) {
        return mModel.getAlreadyChoosedPic(complainDetailEntity);
    }
