...
public class AdActivityPresenter extends AppBaseActivityPresenter<AdActivity> {
    ...
    @OnClick(R.id.img)
    void ad() {
        if (getView().getAdType() == AdViewPagerAdapter.IMAGE_AD_TYPE) {
    ...
    public void onViewInited(Bundle savedInstanceState) {
        adsEntity = getView().getAdsEntity();
//        AdEntity.AdsEntity adsEntity = getView().getAdsEntity();

        if (AdViewPagerAdapter.VIDEO_AD_TYPE.equalsIgnoreCase(getView().getAdType())) {
